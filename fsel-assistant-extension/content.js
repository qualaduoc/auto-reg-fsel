...
function loadConfig() {
  chrome.storage.local.get(['geminiKeys', 'geminiModel', 'autoSolve'], (result) => {
    if (result.geminiKeys && Array.isArray(result.geminiKeys) && result.geminiKeys.length > 0) {
      ...
      addLog(`Đã nhận ${result.geminiKeys.length} API Keys. Model: ${config.geminiModel}`);
    } else {
      // Hỗ trợ fallback nếu thầy chỉ có key cũ dạng chuỗi đơn lẻ
      chrome.storage.local.get(['geminiKey'], (oldResult) => {
        if (oldResult.geminiKey) {
          ...
          addLog('Đã nhận 1 API Key (tương thích ngược).');
        } else {
          addLog('Cảnh báo: Chưa cài đặt API Keys!');
        }
      });
    }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'config_updated') {
    ...
    updateWidgetUI();
    addLog(`Đã cập nhật ${config.geminiKeys ? config.geminiKeys.length : 0} keys mới.`);
    if (config.autoSolve) {
      checkAndSolve();
    }
...
async function solveQuestion(force = false) {
  ...
  try {
    ...
    addLog(`Đang gửi câu hỏi lên Gemini (Model: ${config.geminiModel || 'gemini-1.5-flash'})...`);
    ...
    chrome.runtime.sendMessage({
      ...
      if (response && response.success) {
        ...
        if (!isNaN(answerIdx) && answerIdx >= 0 && answerIdx < clickableElements.length) {
          ...
          addLog(`${keyInfo} Gemini chọn đáp án: [${answerIdx}] "${target.text}"`);
